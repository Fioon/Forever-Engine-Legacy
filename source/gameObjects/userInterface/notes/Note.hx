...
{
        public static var noteHandler:HScript;
	    public static var exposure:StringMap<Dynamic>;
	    public var isGFsing:Bool = false;
        ...
        public function new(strumTime:Float, noteData:Int, noteAlt:Float, ?prevNote:Note, ?sustainNote:Bool = false, ?noteType:String="")
        {
                super(x, y);
                ...
                try
		        {
		            noteHandler.loadModule(Paths.hxs('hscripts/customNotes'), exposure);
		        }
		        catch (e:Dynamic)
		        {
        ...
        **/
        public static function returnDefaultNote(assetModifier, strumTime, noteData, noteType, noteAlt, ?isSustainNote:Bool = false, ?prevNote:Note = null):Note
        {
                ...
                notecheatHandler = new HScript();
                exposure = new StringMap<Dynamic>();
                exposure.set('assetModifier', assetModifier);
                exposure.set('noteData', noteData);
                exposure.set('noteType', noteType);
                exposure.set('newNote', newNote);
                ...
                switch (assetModifier)
                {
                        ...
                        default: // base game arrows for no reason whatsoever
                                newNote.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset('NOTE_assets', assetModifier, Init.trueSettings.get("Note Skin"),
                                        'noteskins/notes'));
                                newNote.animation.addByPrefix('eventScroll', 'arrowRIGHT');
                ...
                //set note type
                if(noteType != ""){
                    newNote.isSpecial = true;
                    switch (noteType)
                    {
                        case "GF Sing":
                            newNote.isGFsing = true;
                            //do nothing LOL                         
                        case "Hurt Note":
                            newNote.frames = Paths.getSparrowAtlas("SpecialNotes/HURTNOTE_assets");
                            newNote.animation.addByPrefix('greenScroll', 'green0');
                            newNote.animation.addByPrefix('redScroll', 'red0');
                            newNote.animation.addByPrefix('blueScroll', 'blue0');
                            newNote.animation.addByPrefix('purpleScroll', 'purple0');
                            ...
                            newNote.antialiasing = true;
                        default:
                            if (noteHandler.exists("returnDefaultNote"))
			                    noteHandler.get("returnDefaultNote")(assetModifier, noteType);
                    }
                }
                //
                if (!isSustainNote)
        ...
        /*public function setNoteType(newNote:Note){
            var Type:String = newNote.noteType;
            if(Type != ""){
                newNote.isSpecial = true;
                switch (Type){
                    case "GF Sing":
                    ...
                    case "Hurt Note":
                            ...
                            newNote.antialiasing = true;
                    default:
                            if (noteHandler.exists("returnNoteSkin"))
			                    noteHandler.get("returnNoteSkin")(Type, newNote.assetModifier);
                }
            }
        
        }*/
        //TODO: set the noteType of Quant Notes
