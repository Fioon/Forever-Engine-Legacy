...
{       
	...
	public var dadWin = new Sprite();
	public var dadScrollWin = new Sprite();
	...
	override public function update(elapsed:Float)
	{
		...
		dadScrollWin.x = (((dadFrame.offset.x) - (dadOpponent.offset.x / 2)) * dadScrollWin.scaleX);
		dadScrollWin.y = (((dadFrame.offset.y) - (dadOpponent.offset.y / 2)) * dadScrollWin.scaleY);
	}

	public function popupWindow(customWidth:Int, customHeight:Int, customX:Int, customY:Int, customName:String, isBorderLess:Bool = false)
	{
		...
		windowDad.x = customX;
		windowDad.y = customY;
		windowDad.stage.color = 0xFF09FF00;
		@:privateAccess
		...
		dadWin.graphics.beginBitmapFill(dadOpponent.pixels, m);
		dadWin.graphics.drawRect(0, 0, dadOpponent.pixels.width, dadOpponent.pixels.height);
		...
		dadScrollWin.scaleX = 1;
		dadScrollWin.scaleY = 1;
		...
		if (generatedMusic && startedCountdown)
		{
			for (strumline in strumLines)
			{
				...
				strumline.allNotes.forEachAlive(function(daNote:Note)
				{
					...
					var noteType:String = daNote.noteType;
					...
					daNote.x = strumline.receptors.members[(Math.floor(daNote.noteData) == -1) ? 0 : Math.floor(daNote.noteData)].x
					...
					if (!daNote.tooLate && daNote.strumTime < Conductor.songPosition - (Timings.msThreshold) && !daNote.wasGoodHit)
					{
						if ((!daNote.tooLate) && (daNote.mustPress))
						{
							if (!daNote.isSustainNote)
							{
                                ...
                                if (modcheatHandler.exists("noteMiss"))
			                        modcheatHandler.get("noteMiss")(daNote.noteType, boyfriend);
			                        
                                if(daNote.isSpecial==true){
	...
	function goodNoteHit(coolNote:Note, character:Character, characterStrums:Strumline, ?canDisplayJudgement:Bool = true)
	{
		...
		if (!coolNote.wasGoodHit)
		{
			...
			{			
				...
				if(coolNote.isSpecial==true){
				    switch (coolNote.noteType)
                    {
                        case "Hurt Note":
                            for (curChar in boyfriendStrumSingCharacters){
					            curChar.playAnim("Hurt", true);
					            curChar.holdTimer = 0;
				            }
				            missNoteCheck((Init.trueSettings.get('Ghost Tapping')) ? true : false, coolNote.noteData, boyfriend, true);
				        default:
	...
	override function beatHit()
	{
		...
		if (Stage.stageHandler.exists("beatHit"))
			Stage.stageHandler.get("beatHit")(curBeat);
	}
	...
	function endSong():Void
	{
		...
		if (Stage.stageHandler.exists("onEndSong"))
			Stage.stageHandler.get("onEndSong")();
