...
class Paths
{
	...
	public static function returnStageGraphic(curStage:String, key:String, ?library:String, ?textureCompression:Bool = false)
	{
		var path = SUtil.getPath() + getPath('stages/$curStage/images/$key.png', IMAGE, library);
		if (FileSystem.exists(path))
		{
			if (!currentTrackedAssets.exists(key))
			{
	...
	inline static public function stageImage(curStage:String, key:String, ?library:String, ?textureCompression:Bool = false)
	{
		var returnAsset:FlxGraphic = returnStageGraphic(curStage, key, library, textureCompression);
		return returnAsset;
	}
	...
	inline static public function getStageSparrowAtlas(curStage:String, key:String, ?library:String)
	{
		var graphic:FlxGraphic = returnStageGraphic(curStage, key, library);
		return (FlxAtlasFrames.fromSparrow(graphic, File.getContent(SUtil.getPath() + file('stages/$curStage/images/$key.xml', library))));
	}

	inline static public function getPackerAtlas(key:String, ?library:String)
	{
		return (FlxAtlasFrames.fromSpriteSheetPacker(image(key, library), SUtil.getPath() + file('images/$key.txt', library)));
	}

	inline static public function getStagePackerAtlas(curStage:String, key:String, ?library:String)
	{
		return (FlxAtlasFrames.fromSpriteSheetPacker(stageImage(key, library), SUtil.getPath() + file('stages/$curStage/images/$key.txt', library)));
	}
}
